    ...
    def __init__(
        ...
        *,
        ...
        if VOICE_PROVIDER == "xai":
            ...
            self.provider = XAIProvider(api_key=XAI_API_KEY, voice=XAI_VOICE)
        elif VOICE_PROVIDER == "openai":
            if not OPENAI_API_KEY:
                raise ValueError("OPENAI_API_KEY not set in environment")
            self.provider = OpenAIProvider(api_key=OPENAI_API_KEY, model=OPENAI_MODEL, voice=persona_manager.get_current_persona_voice())
        else:
