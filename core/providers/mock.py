...
class MockAIProvider(AIProvider):
    ...
    async def send_audio(self, audio_pcm: bytes) -> None:
        """Send raw PCM audio (mock: record the chunk and simulate transcription and response)."""
        self.audio_chunks_sent.append(audio_pcm)

        # Simulate transcription and response after receiving audio
        if len(self.audio_chunks_sent) == 1:  # First chunk, start simulation
            # Transcript
            await self._event_queue.put(ProviderEvent(
                type=ProviderEventType.TRANSCRIPT_DELTA.value,
                data={"text": "Hello, I heard you!"}
            ))
            await self._event_queue.put(ProviderEvent(
            ...
            # Mock audio bytes (empty for simplicity)
            await self._event_queue.put(ProviderEvent(
