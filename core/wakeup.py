...
from .config import CUSTOM_INSTRUCTIONS, OPENAI_API_KEY, OPENAI_MODEL, XAI_API_KEY, VOICE_PROVIDER
...
class WakeupClipGenerator:
    def __init__(self, *, voice: Optional[str] = None, persona_name: str = "default"):
        ...
        if VOICE_PROVIDER == "xai":
            ...
            self.provider = XAIProvider(api_key=XAI_API_KEY, voice=self.voice)
        elif VOICE_PROVIDER == "openai":
            if not OPENAI_API_KEY:
                raise ValueError("OPENAI_API_KEY not set in environment")
            self.provider = OpenAIProvider(api_key=OPENAI_API_KEY, model=OPENAI_MODEL, voice=self.voice)
        else:
